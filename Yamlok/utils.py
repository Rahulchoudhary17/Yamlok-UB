...
from Yamlok import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, SUDO_USERS, bot
from Yamlok.helpers.exceptions import CancelProcess
from var import Var
...
if ENV:
    from Yamlok.uniborgConfig import Config
else:
...
def load_extra(shortname):
    if shortname.startswith("__"):
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = Yamlok.utils
        mod.Config = Config
        mod.borg = bot
        mod.edit_or_reply = edit_or_reply
        # support for paperplaneextended
        sys.modules["Yamlok.events"] = Yamlok.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Yamlok.modules." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def load_pro(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Yamlok/modules/assistant/{shortname}.py")
        name = "Yamlok.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("Successfully imported " + shortname)
        ...
        path = Path(f"Yamlok/modules/assistant/{shortname}.py")
        name = "Yamlok.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        mod.bot = bot
        mod.tgbot = bot.tgbot
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Yamlok.modules.assistant." + shortname] = mod
        LOGS.info("Successfully imported " + shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"Yamlok.modules.{shortname}"
