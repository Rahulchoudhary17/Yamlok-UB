...
from Yamlok.modules.sql_helper.ghdb_sql import in_channels, add_channel, rm_channel, get_all_channels
from Yamlok import CMD_HELP
logs_id = Var.PRIVATE_GROUP_ID


@borg.on(admin_cmd(pattern="forward ?(.*)"))
@borg.on(sudo_cmd(pattern="forward", allow_sudo=True))
...
@borg.on(sudo_cmd(pattern="broadcast", allow_sudo=True))
async def _(event):
  if event.fwd_from:
        return
  if not event.is_reply:
...
@borg.on(sudo_cmd(pattern="listchannels", allow_sudo=True))
